...
lxc-info -n magento || lxc-clone -s -B aufs trusty-base magento

for d in juju mysql magento; do
  lxc-start -d -n $d;
done
...
MG_IP=$(lxc-info -n magento -i | awk '{print $2}')
...
juju add-machine ssh:ubuntu@${MG_IP} #2
...
test -d charms/trusty/magento || git clone https://github.com/mozaroc/charm-magento.git charms/trusty/magento
...
juju deploy --repository=charms/ local:trusty/magento --to 2 || juju deploy --repository=charms/ local:trusty/magento --to 2 || exit 1;
...
juju add-relation haproxy magento

for s in mysql magento haproxy; do
    while true; do
